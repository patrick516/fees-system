...
export default function LoginPage() {
  ...
  // ---------- Loading state (brief, while branding fetches) ----------
  if (brandingLoading) {
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <Loader2 size={28} className="animate-spin text-[#0B1F44]" />
      </div>
    );
  }

  // ---------- Unknown school slug ----------
  if (schoolNotFound || !schoolBranding) {
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center px-4 py-10">
        ...
        <p className="text-sm text-gray-500 mt-2 mb-8 text-center max-w-sm">
          We couldn&apos;t find a school with the code{" "}
          <span className="font-mono font-medium text-gray-700">{slug}</span>.
          Check the code with your school office, or try again.
        </p>
        <button
          onClick={() => router.push("/")}
          className="bg-[#0B1F44] text-white px-6 py-3 rounded-xl font-medium text-sm transition-all duration-300 hover:bg-[#0A1A3A]"
        >
          Try another code
        </button>
    ...
    <div className="min-h-screen bg-white flex flex-col items-center justify-center px-4 py-10">
      ...
      <div className="mb-6 flex flex-col items-center animate-fade-in">
        ...
        {schoolBranding.motto && (
          <p className="text-xs italic text-[#0B1F44]/70 mt-1">
            &ldquo;{schoolBranding.motto}&rdquo;
          </p>
        )}
      </div>
